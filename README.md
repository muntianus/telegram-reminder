...
- `/webdoc` – вывести документацию по формату web_search.
...
Бот включает комплексные механизмы защиты от злоупотреблений:

**Валидация пользовательского ввода:**
- Максимальная длина команд: 2000 символов
- Максимальная длина поисковых запросов: 1000 символов  
- Максимальная длина сообщений чата: 4000 символов
- Минимальная длина поисковых запросов: 2 символа
- Проверка на валидные UTF-8 символы
- Фильтрация опасных управляющих символов

**Защита ресурсов:**
- Кеш поиска ограничен 100 записями с LRU-политикой
- Автоматическая очистка устаревших записей (TTL: 10 минут)
- Таймауты для всех внешних запросов (3 минуты для OpenAI)
- Защита от утечек goroutine в стриминге

**Валидация конфигурации:**
- Проверка диапазонов Telegram Chat ID:
...
1. **Переменные окружения:** Никогда не комитьте `.env` файлы с реальными токенами
2. **Права доступа:** Дайте боту минимально необходимые права в чатах
3. **Мониторинг:** Используйте `LOG_CHAT_ID` для отслеживания ошибок
4. **Обновления:** Регулярно обновляйте зависимости для получения патчей безопасности

📋 **Подробнее:** См. [SECURITY.md](docs/SECURITY.md) для детальных рекомендаций по безопасности.
...
Убедитесь, что эти значения переданы через окружение или указаны в файле `.env`. 

### Ошибки валидации

**Ошибка "Invalid Chat ID":**
- Проверьте, что `CHAT_ID` и `LOG_CHAT_ID` находятся в допустимых диапазонах
- Пользователи: 1 до 2,147,483,647
- Группы/каналы: -2,147,483,648 до -1

**Ошибки "Message too long" или "Query too short":**
- Команды ограничены 2000 символами
- Поисковые запросы: от 2 до 1000 символов
- Сообщения чата: до 4000 символов

**Ошибка "Input contains invalid UTF-8":**
- Убедитесь, что ввод содержит только валидные UTF-8 символы
- Избегайте управляющих символов (кроме новой строки и табуляции)
...
- **[🔒 Безопасность](docs/SECURITY.md)** - Комплексное руководство по безопасности, защите от атак и рекомендациям
- **[⚙️ Конфигурация](docs/CONFIGURATION.md)** - Детальное описание всех настроек и переменных окружения
- **[🔧 Устранение неполадок](docs/TROUBLESHOOTING_GUIDE.md)** - Решения типичных проблем и диагностика

### Примеры конфигурации

- **[Продакшен](docs/examples/.env.production)** - Безопасная конфигурация для боевой среды
- **[Разработка](docs/examples/.env.development)** - Настройки для локальной разработки
- **[Базовый пример](.env.example)** - Стандартный шаблон конфигурации

### Техническая документация

- **[Примеры дайджестов](docs/ENHANCED_DIGEST_EXAMPLES.md)** - Детальные примеры всех типов дайджестов
- **[Стратегия моделей](docs/MODELS_STRATEGY.md)** - Распределение моделей OpenAI по типам задач
- **[Поддерживаемые модели](docs/MODELS.md)** - Полный список совместимых моделей

### 🆘 Нужна помощь?

1. **Проблемы с запуском** → [Устранение неполадок](docs/TROUBLESHOOTING_GUIDE.md)
2. **Вопросы безопасности** → [Руководство по безопасности](docs/SECURITY.md)
3. **Настройка конфигурации** → [Руководство по конфигурации](docs/CONFIGURATION.md)
4. **Технические вопросы** → [GitHub Issues](https://github.com/muntianus/telegram-reminder/issues)
