...
- **Реальный веб-поиск** через DuckDuckGo API
...
- Выполняет веб-поиск через DuckDuckGo API
...
### Функции веб-поиска
- `WebSearch(query)` - выполняет поиск через DuckDuckGo API
...
```yaml
model: gpt-4o  # Основная модель с веб-поиском

tasks:
  - name: crypto_am
    time: "11:00"
    model: gpt-4o  # Полный веб-поиск
  - name: micro_noon
    time: "10:00"
...
1. Используйте `gpt-4o` для важных дайджестов
...
### Проблемы с поиском
```bash
# Проверка доступности DuckDuckGo API
